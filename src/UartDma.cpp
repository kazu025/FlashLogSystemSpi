...
/* -----------------------------------------------------------------------------------------------
	Uart送信
	-----------------------------------------------------------------------------------------------*/
uint16_t UartDma::tx_free_space_locked() const {
	// critical section内で使う前提
	// head == tailのとき空
	// next == tailを満杯判定（実際の有効容量は tx_size_ - 1）
	// tx_sizeは2のべき乗
	return (tx_tail_ - tx_head_ - 1) & (tx_size_ -1);
}
void UartDma::push_bytes_locked(const uint8_t* data, uint16_t len){
	// バッファに空きがある前提でコピーする関数
	for(uint16_t i = 0; i < len; i++){
		tx_buf_[tx_head_] = data[i];
		tx_head_ = (tx_head_ + 1) & (tx_size_ - 1);
	}
}
/* Frame単位送信関数 */
bool UartDma::write_frame(const uint8_t* data, uint16_t len){
	if(data == nullptr || len == 0) return false;
	uint32_t save = save_and_disable_interrupts();
	uint16_t free_space = tx_free_space_locked();
	if(len > free_space){
		tx_drop_frame_count_++;
		tx_drop_bytes_count_ += len;
		return false;
	}
	push_bytes_locked(data, len);
	start_tx_dma_locked();
...
uint32_t UartDma::getTxByteDropCount() const {
	uint32_t save = save_and_disable_interrupts();
	uint32_t v = tx_byte_full_count_;
	restore_interrupts(save);
	return v;
}

uint32_t UartDma::getTxDropFrameCount() const {
	uint32_t save = save_and_disable_interrupts();
	uint32_t v = tx_drop_frame_count_;
	restore_interrupts(save);
	return v;
}
uint32_t UartDma::getTxDropBytesCount() const {
	uint32_t save = save_and_disable_interrupts();
	uint32_t v = tx_drop_bytes_count_;
	restore_interrupts(save);
	return v;
}